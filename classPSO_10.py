...
import copy
import os
from customLoss import R2

class psoDNN10:
    def __init__(self, x_, y_, isLoc):
        self.x = x_
        self.y = y_
        self.modelLayerAmount = 10
        self.isLoc = isLoc
    ...
    def datasetCreating(self):
        ...
        return xTrain, xTest, yTrain, yTest, iTrain, iTest
    
    def modelTraining(self, D1, D2, D3, D4, D5, D6, D7, D8, D9, D10, mapeHistory):
        # model building
        loss = 'mean_absolute_percentage_error'
        optimizer = opti.Adam(learning_rate=0.0002, beta_1=0.95, beta_2=0.999)
        ...
        model.add(layers.Dense(units=D2, activation=('tanh')))
        model.add(layers.Dense(units=D3, activation=('tanh')))
        model.add(layers.Dense(units=D4, activation=('tanh')))
        model.add(layers.Dense(units=D5, activation=('tanh')))
        model.add(layers.Dense(units=D6, activation=('tanh')))
        model.add(layers.Dense(units=D7, activation=('tanh')))
        model.add(layers.Dense(units=D8, activation=('tanh')))
        model.add(layers.Dense(units=D9, activation=('tanh')))
        model.add(layers.Dense(units=D10, activation=('tanh')))
        model.add(layers.Dense(units=self.y[0].shape[0], activation=('linear')))
        model.compile(loss=loss, optimizer=optimizer)
        
        # dataset creating
        xTrain, xTest, yTrain, yTest, iTrain, iTest = self.datasetCreating()
        
        # model training
        model.fit(xTrain, yTrain, validation_data=(xTest, yTest), verbose=0, epochs=200)
    ...
    def plotTrueAndPredicted(self, x, nestedYT, nestedYP, runIdx, category):
        ...
        for i in range(len(nestedYT.T)):
            ...
            r2 = r2_score(nestedYT.T[i], nestedYP.T[i])
            ...
            plt.title(f"{title[i]} (DNN_10layers) [{category}]\nMAPE={mape:.2f}%; R^2={r2:.2f}", fontsize=20)
            plt.grid()
            if self.isLoc:
    ...
    def pso(self, particleAmount, maxIterTime):
        ...
        while IterTime < maxIterTime:
            ...
            for particleIdx in range(len(particlePopulation)):
                ...
                D3 = int(particlePopulation[particleIdx, 2])
                D4 = int(particlePopulation[particleIdx, 3])
                D5 = int(particlePopulation[particleIdx, 4])
                D6 = int(particlePopulation[particleIdx, 5])
                D7 = int(particlePopulation[particleIdx, 6])
                D8 = int(particlePopulation[particleIdx, 7])
                D9 = int(particlePopulation[particleIdx, 8])
                D10 = int(particlePopulation[particleIdx, 9])

                # training result of current particle
                # edit the part below when model is changed
                newFitness[particleIdx], mapeHistory = self.modelTraining(D1, D2, D3, D4, D5, D6, D7, D8, D9, D10, mapeHistory)
            print(newFitness, '\n', mapeHistory)
            ...
            if abs(np.mean(bestPopulationFitness)-min(bestPopulationFitness)) < 0.0001: #convergent criterion
                break

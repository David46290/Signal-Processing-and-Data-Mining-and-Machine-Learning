...
class psoDNN7:
    ...
    # edit the part below when model is changed
    def modelTraining(self, D1, D2, D3, D4, D5, D6, D7, RSN, metricHistory):
        # model building
        ...
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(units=D3, activation=('sigmoid'), kernel_initializer=initializer, bias_initializer="zeros"))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(units=D4, activation=('sigmoid'), kernel_initializer=initializer, bias_initializer="zeros"))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(units=D5, activation=('sigmoid'), kernel_initializer=initializer, bias_initializer="zeros"))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(units=D6, activation=('sigmoid'), kernel_initializer=initializer, bias_initializer="zeros"))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(units=D7, activation=('sigmoid'), kernel_initializer=initializer, bias_initializer="zeros"))
        model.add(layers.Dropout(0.1))
        ...
        try:
            ...
            print(f'RSN number: {RSN}, DNA: {[D1, D2, D3, RSN]}')
            xTrain, xVal, yTrain, yVal = train_test_split(self.xTrain, self.yTrain, test_size=0.2, random_state=RSN)
        # model training
    ...
    def bestModel(self, metricHistory, Gbest):    # To see the performance of the best model
        ...
        self.plotTrueAndPredicted(self.xTest, self.yTest, yTestPredicted, "(DNN_7_PSO) [Test]", self.isLoc)
    ...
    def pso(self, particleAmount, maxIterTime):
        ...
        while IterTime < maxIterTime:
            ...
            for particleIdx in range(len(particlePopulation)):
                ...
                # edit the part below when model is changed
                newFitness[particleIdx], metricHistory = self.modelTraining(D1, D2, D3, D4, D5, D6, D7, RSN, metricHistory)
